...
export default function pending({ pedidos }) {
  ...
  async function fazerPagamento(subtotal) {
    ...
    }).then(resposta => resposta.json()).then(qrCode => {
      setQrCode(qrCode);
      setIsFetched(true)
    });
  }
    ...
    <>
                ...
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  <div className="mt-2 grid grid-rows-4 gap-0">
                    <p className="text-sm text-gray-500">
                      Precisamos do numero do seu CPF para gerarmos o QRCode/Código pix.
                    </p>
                    <p className="text-sm text-gray-500 mb-2">
                      Insira o número abaixo, ao clicar em enviar será redirecionado à uma pagina de pagamento.
                    </p>
                    <div className='border p-2 border-gray-400 rounded-md text-gray-500 mb-4'>
                      <h1>Valor total: R${total}</h1>
                    </div>
                    <TextField value={CPF} onChange={(e) => {
                      setCPF(() => {
                        ...
                        console.log("not if");
                        if (!isNaN((e.target.value).replace('.', '').replace('-', ''))) {
                          return (e.target.value)
                        } else {
                          console.log("é letra");
                          return CPF
                    ...
                    }} type="text" size='small' label="CPF" className='h-8 outline-none text-blue-900 focus-visible:ring-1 focus-visible:ring-blue-500 focus-visible:ring-offset-0 rounded-md border border-gray-400' onBlur={isEnough} helperText="O CPF deve ter 14 digitos" error={erro} />
                  </div>
                  ...
                  <div className="mt-4 flex justify-between">
                    {(Object.keys(qrCode).length > 0) &&
                      <button onClick={copiar} className="inline-flex justify-center rounded-md border border-transparent bg-blue-300 px-4 py-2 text-sm font-medium text-blue-900 hover:bg-blue-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2">Copiar código Pix</button>}
                    {isFetched ?
                      <button
                        type="button"
      ...
      <div className='container m-auto p-4'>
        ...
        <div className='border-t-2'>
          ...
          {pedidos.ID ? pedidos.ID == 500 ? <>{pedidos.DATA}</> : <></> : pedidos.map((pedido) => (
            <div key={pedido.ID} className={`${quick.className} mt-5 border-gray-300 border p-4 rounded-lg`}>
              ...
              <div>
                  ...
                  {pedido.DATA.listaProdutos.map((produto) => (
                    <li key={produto.nome}>
                      <div className='grid grid-cols-10 gap-2 items-center sm:border-x sm:border-gray-300'>
                        <img src={produto.imagem} className='col-span-3 sm:max-h-40 sm:col-span-3 place-self-center'/>
                ...
                <div className='grid grid-cols-6 grid-rows-2 items-center'>
                  <div className='pt-2 col-span-6'>Pedido efetuado em {(new Date(new Timestamp(pedido.DATA.criacao.seconds, pedido.DATA.criacao.nanoseconds).toDate()).toLocaleDateString())}</div>
                  <div className='col-span-4 align-text-bottom'><ExclamationCircleIcon className='h-4 text-yellow-500 inline' /> {pedido.DATA.status}</div>
                  <button onClick={() => openModal(pedido.DATA.subtotal)} className='border border-gray-300 col-span-2 rounded-md bg-indigo-600 text-white py-1'>Efetuar Pagamento</button>
                </div>
              </div>
            </div>))}
        </div>
